...
- Tips：无 API Key 时返回本地题目模板；配置 DeepSeek 后可生成更贴近输入内容的建议。
- 选择题识别：可识别带 A、B、C、D 选项的题目，并按每个选项分别生成模型。
- 理解确认：复杂题或选择题会先展示题型、关系、视图模式和简略预览，用户确认后再生成。

### 本地规则建模

- 3D 空间几何：正方体、长方体、三棱锥、四棱锥 / 正四棱锥。
...
- 使用记录：前端要求学生登录后才能生成，后端会在有用户 token 时记录个人用量。
- 邀请码：管理员可创建邀请码，设置最大使用次数，并把每日 AI 调用额度字段写入学生账号。
...
- 本地 Node 后端会按后台配置的全局 `dailyLimit` 拦截 `/api/parse` AI 调用。
- Cloudflare Worker 版本已经保存 `dailyLimit` 和学生 `dailyAiLimit` 字段，并记录 D1 用量；付费公网版本上线前，需要再确认 Worker 是否按这些字段做硬性拦截。
