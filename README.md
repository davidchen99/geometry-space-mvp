...
要求 Node.js 18 或更高版本。页面中的 Three.js 和 lucide 图标通过 CDN 加载，图片 OCR 会按需加载 Tesseract.js CDN，浏览器需要能访问外网 CDN。
...
- 图片输入：支持上传题目图片，或把截图直接粘贴到输入框；上传后可删除重选。浏览器端 OCR 会先做图片增强和质量判断，低质量识别会拦截自动填入，避免错字直接生成乱码模型。
- 选择题识别：可识别带 A、B、C、D 选项的题目，并按每个选项分别生成模型。
- 显式坐标：支持 `A(1,2,3)` / `B(4,2,3)` 这类点坐标，生成时优先按题目坐标准确放置。
- 常见数学符号：输入和 Tips 支持更接近考试题的 `O₁`、`A₁`、`√3`、`×`、`÷`、`⊥`、`∥` 写法；内部会转换解析，展示时优先保留题目里的根号表达。

### 本地规则建模

- 3D 空间几何：正方体、长方体、直棱柱、正棱锥、正四面体、三棱锥 / 三角锥、四面体、四棱锥 / 正四棱锥、P-ABC / P-ABCD 这类棱锥记法；模型会尽量保持题目点名和模型标签一一对应。
- 3D 曲面 / 旋转体：圆柱、圆锥、圆台、球、椭球面、单叶 / 双叶双曲面、椭圆抛物面、双曲抛物面、椭圆柱面、双曲柱面、抛物柱面；缺少尺寸时使用默认尺寸并在关系区提示。
- 组合模型：支持长方体 / 正方体与圆形、圆柱、球、三棱锥、四棱锥、四面体等放在同一坐标系中展示；圆柱穿过长方体、多个截面圆、多面体相交等场景会用交线、截面圆或高亮面突出相交部分，多面体组合会用不同透明度、粗线和高亮色区分主体与相交区域，并尽量保持每个形体完整显示。
- 2D 平面几何：三角形、圆、椭圆、抛物线、双曲线、直线、轨迹 / 动点示意。
- 文本特征：边长、指定线段长度、中点、交点 / 底面中心、重心、连接线、垂直 / 平行关系。
- 自动信息：点坐标、线段长度、面面积、曲面采样面积、体积、底面方程、高线方程、边或连接线的参数方程。
...
- PWA：支持通过浏览器安装到桌面或手机桌面，使用独立窗口打开；核心静态资源会通过 service worker 缓存，方便二次打开。
